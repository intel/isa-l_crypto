...
struct aes_context {
        ...
        void (*const processor)(struct aes_context *pCtx, char *plaintext, char *ciphertext,
                                uint64_t len);
...
static uint64_t
aes_thread_func(int32_t id, struct aes_context *pCtx)
{
        uint32_t i = 0, j = 0;
...
static int
cbc_dec_pre(struct aes_context *p)
{
...
static void
cbc_dec_post(struct aes_context *p)
{
        struct cbc_context *pCtx = (struct cbc_context *) p;
...
static void
cbc_dec_proc(struct aes_context *p, char *plaintext, char *ciphertext, uint64_t len)
{
        struct cbc_context *pCtx = (struct cbc_context *) p;
...
static int
xts_enc_pre(struct aes_context *p)
{
...
static void
xts_enc_post(struct aes_context *p)
{
        return;
}

static void
xts_enc_proc(struct aes_context *p, char *plaintext, char *ciphertext, uint64_t len)
{
        struct xts_content *pCtx = (struct xts_content *) p;
...
gcm_enc_pre(struct aes_context *p)
{
        ...
        if (pCtx->base.bits == 128)
                isal_aes_gcm_pre_128(pCtx->key, &pCtx->gkey);
        else
                isal_aes_gcm_pre_256(pCtx->key, &pCtx->gkey);

        return 0;
}

static void
gcm_enc_post(struct aes_context *p)
{
        struct gcm_context *pCtx = (struct gcm_context *) p;
...
static void
gcm_enc_proc(struct aes_context *p, char *plaintext, char *ciphertext, uint64_t len)
{
        struct gcm_context *pCtx = (struct gcm_context *) p;
