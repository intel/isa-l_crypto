 ...
 * @details
 *	usage: taskset -c <cpu_index1,cpu_index2,...> isal_multithread_perf -m <algorithm name> -n
 *<thread num> eg: taskset -c 0-9,20-29 ./isal_multithread_perf -m md5_mb -n 10
 */
...
alg_method algs[] = { { "md5", md5_ossl_func, MD5_MAX_LANES },
                      { "md5_mb", md5_mb_func, MD5_MAX_LANES },
                      { "sha1", sha1_ossl_func, SHA1_MAX_LANES },
                      { "sha1_mb", sha1_mb_func, SHA1_MAX_LANES },
                      { "sha256", sha256_ossl_func, ISAL_SHA256_MAX_LANES },
                      { "sha256_mb", sha256_mb_func, ISAL_SHA256_MAX_LANES },
...
void
notice(char *appname, alg_method *alg_choose_p)
{
        int i = 0;
        ...
        printf("\tverbose output is %d\n"
               ...
               verbose, run_secs, num_threads, buflen / 1024, prememcpy, postmemcpy,
               alg_choose_p->name);
}
...
main(int argc, char **argv)
{
        ...
        for (i = 0; i < num_threads; i++) {
                ...
                ret = pthread_create(&clients[i], NULL, alg_choose.thread_func, (void *) &id[i]);
